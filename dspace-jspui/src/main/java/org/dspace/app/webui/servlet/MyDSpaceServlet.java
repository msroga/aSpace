...
//         JSPManager.showJSP(request, response, "/mydspace/perform-task.jsp");
   ...
   private void processExportArchive(Context context,
                                     HttpServletRequest request, HttpServletResponse response, boolean migrate)
           ...
           SQLException, AuthorizeException
   {
      ...
      if (buttonPressed.equals("submit_mapfile"))
      {
         ...
         try
         {
            mapFilePath = ItemImport.getImportUploadableDirectory(
                    context.getCurrentUser().getID()) + File.separator + uploadId + File.separator + "mapfile";
         }
         catch (Exception e)
         {
           ...
           SQLException, AuthorizeException
   {
      ...
      boolean displayMemberships = ConfigurationManager
              .getBooleanProperty("webui.mydspace.showgroupmemberships", false);
